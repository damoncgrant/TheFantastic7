...
function RecruiterCandidateSwiper({ candidates, onCandidateReviewed }) {
  ...
  const commitSwipe = useCallback(async (direction, reason = '') => {
    ...
    try {
      await reviewCandidateApplication(current.application_id, direction, reason);
  ...
  return (
    <>
      <RecruiterHeader
        ...
        title="Candidates"
        description="Swipe right to invite a candidate to interview, or left to reject an application."
      />
      {content}
