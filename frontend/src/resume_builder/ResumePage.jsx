...
export default function ResumePage() {
  ...
  async function deleteResume(id) {
    ...
    try {
      ...
      await loadResumes();
    } catch (err) { setError(err.message || 'Could not delete that resume.'); }
  }
...
function LatexResumeEditor({ busy, initialName, initialLatex, onSave, onRender, onClose }) {
  ...
  const save = async () => { await onSave({ name: name.trim() || 'Imported resume', latex }); setSaved(true); };
  const saveAndRender = async () => { await save(); onRender(latex); };
  return <section className="builder" aria-label="LaTeX resume editor"><div className="builder-heading"><div><h2>Edit imported LaTeX</h2><p>Your source is saved with this resume and can be edited or rendered again.</p></div><button className="secondary" onClick={onClose} disabled={busy}>Back to resumes</button></div><div className="builder-scroll"><label className="field"><span>Resume name</span><input value={name} onChange={(event) => setName(event.target.value)} /></label><label className="field latex-source"><span>LaTeX source</span><textarea value={latex} onChange={(event) => setLatex(event.target.value)} rows="20" spellCheck="false" /></label></div><div className="builder-actions">{saved && <span className="save-status" role="status">Saved</span>}<button className="secondary" onClick={save}>Save resume</button><button className="render-button" onClick={saveAndRender} disabled={busy}>{busy ? 'Rendering…' : 'Save & render'}</button></div></section>;
}
